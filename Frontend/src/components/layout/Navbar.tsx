...
export default function Navbar({ user, onLogout, customers = [], transactions = [], auditLogs = [] }: NavbarProps) {
  ...
  const navLinks = ({
    ...
    soc: [
      ...
      { path: "/intelligence", label: "Fraud Intel",      icon: Laptop },
      { path: "/audit",        label: "Audit Center",     icon: FileText },
    ],
    admin: [
      { path: "/dashboard",    label: "Dashboard",          icon: LayoutDashboard },
      { path: "/users",        label: "User Management",    icon: Users },
      { path: "/roles",        label: "Roles & Permissions",icon: UserCheck },
      { path: "/config",       label: "Configuration",      icon: Laptop },
      { path: "/logs",         label: "System Logs",        icon: FileText },
    ],
  } as Record<string, Array<{ path: string; label: string; icon: any }>>)[role] || [];

  return (
    <nav className="bg-[#0a1628] text-white select-none border-b border-slate-800 relative z-50">
      {/* Row 1: Brand, Desktop KPIs, Controls */}
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
        ...
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="flex min-w-0 flex-col">
            <span className="font-extrabold text-[11px] tracking-tight text-white font-mono uppercase whitespace-nowrap max-[380px]:hidden">Sach Ka Kavach</span>
            <span className="hidden min-[420px]:block text-[8px] text-saffron-400 font-mono tracking-widest leading-none mt-0.5">BHARAT TRUST GRID</span>
          </div>
        </div>
